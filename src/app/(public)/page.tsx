...
export default async function HomePage() {
  ...
  const recentEssays = allPosts.filter((p) => p.id !== leadEssay?.id);

  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-16">
      {/* 1. Lead Featured Essay (Opens Directly with the publication) */}
      {leadEssay ? (
      ...
      {/* 2. Field Notes & Quick Observations ("From My Desk") */}
      ...
      {/* 3. Recent Essays & Investigations Feed */}
      <section className="space-y-6">
        ...
        {recentEssays.length === 0 && !leadEssay ? (
          <div className="p-12 text-center rounded-3xl bg-white dark:bg-[#141a24] border border-stone-200 dark:border-stone-800 space-y-2">
            <p className="font-bold text-base text-stone-800 dark:text-stone-200">No published articles yet</p>
      ...
      {/* 4. "Now" / What Khophi is Working on & Reading */}
